...
PageTableWalker::PageTableWalker(std::string v1, uint32_t cpu, double freq_scale, std::vector<std::pair<std::size_t, std::size_t>> pscl_dims, uint32_t v10,
    ...
    MemoryRequestProducer(ll), NAME(v1), RQ_SIZE(v10), MSHR_SIZE(v11), MAX_READ(v12), MAX_FILL(v13), HIT_LATENCY(latency),
...
#ifdef IPCP_L2_DEBUG
    if (fill_mshr.ip == 6214471 && fill_mshr.v_address == 7744384) {
        std::cout << NAME << "PageTableWalker return_data" << std::endl;
        std::cout << "PageTableWalker return_data " << fill_mshr.address << std::endl;
        std::cout << "PageTableWalker return_data " << fill_mshr.to_return.size() << std::endl;
        std::cout << typeid(*(fill_mshr.to_return[0])).name() << std::endl;
    }
#endif
    if constexpr (champsim::debug_print) {
